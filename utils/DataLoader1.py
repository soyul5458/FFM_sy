...
class data_loader() :
    ...
    def get_data(self, data_pick, to , tde_pick = None, pl_pick = None, ratio=None) : 
        # data_num의 데이터를 뽑음.
        # data_pick = '1':add2022
        # tde_pick = 't': train, 'd':devel, 'e':eval (ex 'td')  ## 해당 데이터 뽑음
        # pl_pick = 'l': lf, 'p':pf   ## LF, PF 중 어떤 트랙 데이터를 추가할지
        ...
        ## load ASV2019
        if '2' in data_pick :
            res = load_asv2019(self.path['asv2019'], tde_pick, pl_pick, ratio) 
...
def load_asv2019(path2019, tde, pl, ratio) :
    # pick = 't': train, 'd':devel, 'e':eval
